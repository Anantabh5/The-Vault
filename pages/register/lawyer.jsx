...
const RegistrationForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    securityPin: '',
    advocateEnrollmentNumber: '', // New field for Advocate Enrollment Number
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await router.push(`/lawyer/${response.data.id}`); // Assuming the response contains an 'id' field
    } catch (error) {
      console.error(error);
      alert('Registration failed');
    }

    Cookies.set('lawyerid', formData.advocateEnrollmentNumber);
      ...
      <Layout>
        <div className="container mt-5">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              <label htmlFor="advocateEnrollmentNumber">Advocate Enrollment Number</label>
              <input
                  type="text"
                  className="form-control"
                  id="advocateEnrollmentNumber"
                  name="advocateEnrollmentNumber"
                  value={formData.advocateEnrollmentNumber}
                  onChange={handleInputChange}
                  required
              />
            </div>
